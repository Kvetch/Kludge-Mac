...
class CmdLnr(object):
    ...
    def run_acmd(self,cmdargs,ofile):
        ...
        try:
            #proc = subprocess.Popen(cmdargs + ";" + cmdargs + " >&2", shell=True, stderr=subprocess.PIPE, stdout=subprocess.PIPE) #### stderr redirect
            # proc = subprocess.Popen(str(cmdargs), shell=True, stdout=subprocess.PIPE)
